...
`/arckit-fr:fr-rgpd` generates a French CNIL-specific GDPR layer assessment supplementing `/arckit-eu:eu-rgpd`. It covers CNIL délibérations, health data (HDS), cookie rules, age 15 for minors, and CNIL enforcement patterns.
...
```bash
/arckit-fr:fr-rgpd Assess CNIL compliance for <project or processing activity>
```

Output: `projects/<id>/ARC-<id>-CNIL-v1.0.md`

> **Run after**: `/arckit-eu:eu-rgpd` — this command adds the French layer on top.
...
| French layer | CNIL-specific assessment | `/arckit-fr:fr-rgpd` |
...
- **Scope**: Supplements `/arckit-eu:eu-rgpd`. Do not run in isolation — EU GDPR baseline must be established first.
