...
```bash
/arckit-au:au-dss <project ID or service description>
```
...
Feeds the assurance package alongside `/arckit-au:au-pia` (Privacy Act compliance) and `/arckit-au:au-e8-posture` (cyber baseline). Pre-Live and Live phase artefacts cross-reference DSS conformance evidence in `/arckit-au:au-disp-attestation` for supplier-side accreditation.
