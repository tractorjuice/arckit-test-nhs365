...
`/arckit-au:au-ndb-playbook` generates an operational response playbook under the OAIC Notifiable Data Breaches scheme (Privacy Act 1988 Part IIIC). It captures the assessment criteria for an eligible data breach, the 30-day investigation clock, the notification decision logic, the workflow for notifying affected individuals and the OAIC, and the post-incident review obligations including remediation tracking and lessons-learned capture.
...
| Artefact | Purpose |
|----------|---------|
| `/arckit-au:au-pia` output (`ARC-<id>-AUPIA-v1.0.md`) | Personal information inventory; eligible-data-breach criteria already drafted |
...
```bash
/arckit-au:au-ndb-playbook <project ID or service description>
```
...
Tied tightly to `/arckit-au:au-pia` (PIA establishes the eligible-data-breach criteria) and the operational runbooks (`/arckit:operationalize`). Cited as evidence in `/arckit-au:au-disp-attestation` (DISP information protection domain) and `/arckit-au:au-pspf` (PSPF Outcome 4 incident management).
