...
```bash
/arckit-au:au-pia <project ID or service description>
```
...
Foundational input to `/arckit-au:au-ndb-playbook` (NDB playbook uses the personal information inventory and breach decision criteria), `/arckit-au:au-disp-attestation` (DISP information protection domain cites PIA evidence), and `/arckit-au:au-ai-assurance` (AI Assurance cross-references automated decision-making notification posture).
