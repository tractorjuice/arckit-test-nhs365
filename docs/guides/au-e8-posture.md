...
```bash
/arckit-au:au-e8-posture <project ID or service description>
```
...
- **ASD Information Security Manual (ISM)** — control-level detail referenced by E8 strategies (cross-reference via `/arckit-au:au-ism-controls`)
...
Foundational input to `/arckit-au:au-disp-attestation` (DISP Member self-attestation pack consolidates E8 evidence), `/arckit-au:au-pspf` (PSPF cross-references E8 baselines), and `/arckit-au:au-ism-controls` (ISM Statement of Applicability draws from the same control evidence).
