...
| `/arckit-au:au-e8-posture` output (`ARC-<id>-AUE8-v1.0.md`) | Cyber baseline that the ISM SoA cross-references |
...
```bash
/arckit-au:au-ism-controls <project ID or service description>
```
...
Foundational input to `/arckit-au:au-disp-attestation` (DISP Information & Cyber domain draws heavily from the ISM SoA), `/arckit-au:au-pspf` (PSPF Outcome 4 cites ISM SoA evidence), and feeds the IRAP assessor scoping package.
