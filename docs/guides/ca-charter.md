...
```bash
/arckit-ca:ca-charter <project ID or service description>
```
...
- A FITAA-class app is being designed — Charter §2 expression and association analysis is unconditional after `/arckit-ca:ca-fitaa`.
...
- **`ca-fitaa`** — Charter §2 expression and association analysis is a mandatory companion to FITAA; the FITAA artefact references this review and the public-vs-protected severance design feeds the s.2 mitigation register. If `/arckit-ca:ca-fitaa` has been run, `/arckit-ca:ca-charter` is not optional.
